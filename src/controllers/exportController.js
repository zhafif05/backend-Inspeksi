...
const REPAIR_EQUIPMENT_TEMPLATE_PATH = path.join(__dirname, '../../DRAFT_LAPORAN_HASIL_PERBAIKAN_LAB_DAN_BENGKEL(1).docx');
...
function buildDamageTableRows(tableXml, rows) {
...
function fillRepairTemplateBody(bodyTemplate, lab, repairItems, tahun, semester, zip, imageState, skipHeader = false) {
  let body = bodyTemplate;
  body = removeInitialPageBreaks(body);
  body = forceReportSectionToNewPageRepair(body); // versi khusus repair
  body = removeExtraPageBreaks(body);

  body = replaceTextAcrossRuns(body, 'PERIODE PELAPORAN: SEMESTER GENAP 2024/2025', `PERIODE PELAPORAN: SEMESTER ${semester} ${tahun}`);
  body = replaceTextAcrossRuns(
    body,
    "Nama Lab./Bengkel",
    lab.nama_lab
  );
  ...
  if (firstTableMatch) {
    body = body.replace(firstTableMatch[0], buildRepairTableRows(firstTableMatch[0], repairItems));
  }

  return body;
}

function alignCellWidthsToGrid(tableXml) {
  let gridWidths = [...tableXml.matchAll(/<w:gridCol w:w="(\d+)"\s*\/>/g)].map(m => Number(m[1]));
  if (gridWidths.length === 0) return tableXml;
  const MIN_FIRST_COL = 850;
  if (gridWidths[0] < MIN_FIRST_COL) {
    const deficit = MIN_FIRST_COL - gridWidths[0];
    let widestIdx = 0;
    for (let i = 1; i < gridWidths.length; i++) {
      if (gridWidths[i] > gridWidths[widestIdx]) widestIdx = i;
    }
    if (widestIdx !== 0 && gridWidths[widestIdx] - deficit > 0) {
      gridWidths[widestIdx] -= deficit;
      gridWidths[0] = MIN_FIRST_COL;
    }
  }
  let colIdxForGrid = 0;
  tableXml = tableXml.replace(/<w:gridCol w:w="\d+"\s*\/>/g, () => {
    const w = gridWidths[colIdxForGrid++];
  ...
  const newRows = rows.map(row => {
    ...
    const newCells = cells.map(cell => {
      const spanMatch = cell.match(/<w:gridSpan w:val="(\d+)"\s*\/>/);
      const span = spanMatch ? Number(spanMatch[1]) : 1;
      const width = gridWidths.slice(colIndex, colIndex + span).reduce((a, b) => a + b, 0) || 1000;
      colIndex += span;

      if (/<w:tcW[^/]*\/>/.test(cell)) {
        return cell.replace(/<w:tcW[^/]*\/>/, `<w:tcW w:w="${width}" w:type="dxa"/>`);
      }
      return cell.replace('<w:tcPr>', `<w:tcPr><w:tcW w:w="${width}" w:type="dxa"/>`);
    });

    return replaceRowCellBlock(row, newCells.join(''));
  });

  return tableXml.replace(rows.join(''), newRows.join(''));
}
...
function fillDamagedEquipmentTemplateBody(bodyTemplate, lab, damagedItems, tahun, semester, zip, imageState, skipHeader = false) {
  ...
  if (!skipHeader) {
    body = replaceTextAcrossRuns(body, 'PERIODE PELAPORAN: SEMESTER GENAP 2024/2025', `PERIODE PELAPORAN: SEMESTER ${semester} ${tahun}`);
    body = replaceTextAcrossRuns(body, 'Surabaya, XX-XX-20XX', `Surabaya, ${formatDate(new Date())}`);
  }
  ...
  const firstTableMatch = body.match(/<w:tbl>[\s\S]*?<\/w:tbl>/);
  if (firstTableMatch) {
    let filledTable = buildDamageTableRows(firstTableMatch[0], rows);
    filledTable = alignCellWidthsToGrid(filledTable); // sinkronkan lebar sel dengan tblGrid
    body = body.replace(firstTableMatch[0], filledTable);
  }
...
async function buildRepairLabsData(tahun, semester) {
...
async function getRepairEquipmentData(inspections) {
  ...
  const [rows] = await pool.query(
    `SELECT
      ...
      JOIN items it ON dr.item_id = it.id
      WHERE dr.inspection_id IN (?)
      ORDER BY it.nama_barang ASC, dr.created_at ASC, dr.id ASC`,
    [inspectionIds]
  );
  // menyertakan foto.
  const grouped = new Map();
  for (const row of rows) {
    const key = `${row.item_id}_${row.inspection_id}`;
    if (!grouped.has(key)) {
      grouped.set(key, {
        ...
        status_perbaikan: row.status_peralatan || null,
      ...
      entry.status_perbaikan = row.status_peralatan || entry.status_perbaikan;
      entry.keterangan = row.keterangan || entry.keterangan;
      entry.created_at = row.created_at || entry.created_at;
      if (row.foto) {
        entry.foto = row.foto; // hanya ditimpa kalau row ini MEMANG punya foto
      }
    }
  }
...
async function cloneTemplateWithRepairPages(labsData, tahun, semester) {
  const templateBuffer = fs.readFileSync(REPAIR_EQUIPMENT_TEMPLATE_PATH);
  ...
  let bodyTemplate = lastSectPrMatch
    ? bodyContent.slice(0, lastSectPrMatch.index) + bodyContent.slice(lastSectPrMatch.index + lastSectPrMatch[0].length)
    : bodyContent;

  // --- INI YANG BERUBAH: pakai pencarian tahan-run-terpecah ---
  const paragraphStart = findParagraphStartForFlattenedText(bodyTemplate, 'LAPORAN PER SEMESTER HASIL PERBAIKAN');
  ...
  const pages = labsData.map((entry, labIndex) => {
    ...
    if (labIndex === 0) {
      ...
      pageContent = fillRepairTemplateBody(fullTemplate, entry.lab, entry.repairItems, tahun, semester, zip, imageState, false);
    } else {
      pageContent = fillRepairTemplateBody(bodyWithoutHeader, entry.lab, entry.repairItems, tahun, semester, zip, imageState, true);
    }

    return labIndex < labsData.length - 1 ? `${pageContent}${pageBreak()}` : pageContent;
  }).join('');
...
async function getRepairEquipmentData(inspections) {
  ...
  // Iterasi terurut created_at ASC: status/keterangan mengikuti laporan
  // TERBARU (row terakhir yang diproses untuk key yang sama menang).
  // Foto tidak lagi diambil/disimpan sama sekali.
  const grouped = new Map();
  for (const row of rows) {
    const key = `${row.item_id}_${row.inspection_id}`;
    if (!grouped.has(key)) {
...
function buildRepairTableRows(tableXml, repairItems) {
  const rowMatches = tableXml.match(/<w:tr[\s\S]*?<\/w:tr>/g) || [];
  ...
  return tableXml.replace(rowMatches.join(''), allRows);
}

function getRunTextSpans(xml) {
  const spans = [];
  ...
  while ((m = re.exec(xml)) !== null) {
    spans.push({
      matchStart: m.index,
      innerStart: m.index + `<w:t${m[1]}>`.length,
      innerEnd: m.index + m[0].length - '</w:t>'.length,
      text: m[2]
    });
  }
  return spans;
...
function findFlattenedMatch(spans, searchText) {
  ...
  for (let i = 0; i < spans.length; i++) {
    ...
    if (startSpanIdx === -1 && idx < pos + len) startSpanIdx = i;
    if (endSpanIdx === -1 && endIdx <= pos + len) { endSpanIdx = i; break; }
    pos += len;
  }

  if (startSpanIdx === -1 || endSpanIdx === -1) return null;
  return { startSpanIdx, endSpanIdx, matchStart: spans[startSpanIdx].matchStart };
}

function replaceTextAcrossRuns(xml, searchText, replacementText) {
  const spans = getRunTextSpans(xml);
  const match = findFlattenedMatch(spans, searchText);
  if (!match) return xml;

  const { startSpanIdx, endSpanIdx } = match;
  let result = xml;
  for (let i = endSpanIdx; i >= startSpanIdx; i--) {
    const span = spans[i];
    const newInner = (i === startSpanIdx) ? escapeXml(replacementText) : '';
    result = result.slice(0, span.innerStart) + newInner + result.slice(span.innerEnd);
  }
  return result;
}

// Tambahkan fungsi baru ini (di dekat replaceTextAcrossRuns)
function replaceAllTextAcrossRuns(xml, searchText, replacementText) {
  let result = xml;
  while (true) {
    const next = replaceTextAcrossRuns(result, searchText, replacementText);
    if (next === result) break; // tidak ada lagi kemunculan yang cocok
    result = next;
  }
  return result;
}

function findParagraphStartForFlattenedText(xml, searchText) {
  const spans = getRunTextSpans(xml);
  const match = findFlattenedMatch(spans, searchText);
  if (!match) return -1;

  const targetPos = match.matchStart;
  const paragraphOpenRegex = /<w:p(?=[\s/>])/g;
  let paragraphStart = -1;
  let m;
  while ((m = paragraphOpenRegex.exec(xml)) !== null) {
    if (m.index >= targetPos) break;
    paragraphStart = m.index;
  }
  return paragraphStart;
}
function forceReportSectionToNewPageRepair(body) {
  const paragraphStart = findParagraphStartForFlattenedText(body, 'LAPORAN PER SEMESTER P');
  if (paragraphStart === -1) return body;
...
async function getDamageReportForInspection(inspectionId) {
  const [reports] = await pool.query(
    ...
    JOIN inspection_results ir
      ON dr.inspection_result_id = ir.id
  ...
  const firstWithPhoto =
    reports.find(r => r.foto) || latest;

  return {
    tanggal_ditemukan:
      latest.created_at ||
      latest.result_updated_at ||
      null,

    // tetap gabungan semua laporan
    kondisi_kerusakan: joinDistinct(
      reports.map(r => r.kondisi_kerusakan)
    ),

    penyebab_kerusakan: joinDistinct(
      reports.map(r => r.penyebab_kerusakan)
    ),

    // hanya ambil data terbaru
    status_peralatan:
      latest.status_peralatan || "-",

    foto:
      firstWithPhoto.foto || null
  };
}
...
function ensureFixedTableLayout(xml) {
  return xml.replace(/<w:tblPr>([\s\S]*?)<\/w:tblPr>/g, (match, inner) => {
    if (inner.includes('<w:tblLayout')) return match; // sudah ada, jangan dobel
...
async function cloneTemplateWithDamagedEquipmentPages(labsData, tahun, semester) {
  ...
  const nextDocumentXml = ensureFixedTableLayout(ensureDocumentNamespaces(documentXml.replace(
    /<w:body[\s\S]*?<\/w:body>/,
    `<w:body>${pages}${sectPr}</w:body>`
  )));
...
const exportRepairEquipmentReport = async (req, res, next) => {
  try {
    const { tahun, semester } = req.query;
    const normalizedSemester = normalizeSemester(semester);
    ...
    if (!fs.existsSync(REPAIR_EQUIPMENT_TEMPLATE_PATH)) {
      return res.status(500).json({
        success: false,
        message: 'Template laporan perbaikan peralatan tidak ditemukan.'
      });
    }

    const labsData = await buildRepairLabsData(Number(tahun), normalizedSemester);

    if (labsData.length === 0) {
      return res.status(400).json({
        success: false,
        message: 'Tidak ada data perbaikan peralatan.'
      });
    }

    const buffer = await cloneTemplateWithRepairPages(labsData, Number(tahun), normalizedSemester);
    const filename = `Laporan_Peralatan_Perbaikan_${normalizedSemester}_${tahun}.docx`;
...
const getExportReportStatus = async (req, res, next) => {
  try {
    const { tahun, semester } = req.query;

    const normalizedSemester = normalizeSemester(semester);

    const damaged = await buildDamagedLabsData(
      Number(tahun),
      normalizedSemester
    );

    const repair = await buildRepairLabsData(
      Number(tahun),
      normalizedSemester
    );
    ...
    return res.json({
      success: true,
      hasDamagedReport: damaged.length > 0,
      hasRepairReport: repair.length > 0,
    });
  } catch (err) {
    next(err);
  }
